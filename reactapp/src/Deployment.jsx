    ...
    deploy() {
        ...
        if (web3) {
            web3.eth.getAccounts().then((accounts) => {
                ...
                if (account) {
                    deploy(+ether, arbiter, beneficiary, account).on('transactionHash', (tx) => {
                        this.setState({transactions: this.state.transactions.concat(tx)})
                    }).then(({options}) => {
                        const {address} = options;
                        alert(`Transaction was successful! Deployed to ${address}.`)
                    }).catch((err) => {
                        alert(err);
                    });
                }
                ...
                <ul className="list-group py-2">
                    {
                        transactions.map(tx => {
                            return (
                                <li className="list-group-item" key={tx}>
                                    Transaction Posted: {tx}
                                </li>
                            )
                        })
                    }
